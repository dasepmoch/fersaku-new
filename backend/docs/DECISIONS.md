...
Source of truth for irreversible money/auth/schema choices: `docs/BACKEND_PRODUCTION_TASKS.md` (esp. §0, §14.5, §15 BE-000, §16, §18), `docs/BACKEND_HANDOFF.md`, `ARCHITECTURE.md`. Active production + dual-provider execution: `TASK/PROD/*`.
...
| [ADR-0002](adr/ADR-0002-one-xendit-account.md) | One Xendit account for QRIS payment + disbursement; no Duitku/failover (**payment half superseded by ADR-0008**) | Accepted | 2026-07-16 |
...
| [ADR-0008](adr/ADR-0008-duitku-payment-xendit-disbursement.md) | Dual provider: Duitku payment (QRIS) / Xendit disbursement | Accepted | 2026-07-19 |

ADRs 0001–0007 were **Accepted** as of BE-000 completion. ADR-0008 is **Accepted** (PROD-A10) and supersedes ADR-0002 for **QRIS payment create/callback only**; disbursement remains Xendit. Do not reopen locked decisions; amend only via a new ADR + versioned release when product/security requires it.

Next work: **PROD program / PROD-A20** — Config: `PAYMENT_PROVIDER` + `DISBURSEMENT_PROVIDER` (not BE-002).
