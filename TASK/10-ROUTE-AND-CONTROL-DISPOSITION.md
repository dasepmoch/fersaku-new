...
| `AUTH_REQUIRED` | Pre-auth/recent-proof required | Existing auth/security dialog/form composition; no command replay otomatis setelah proof kecuali contract explicitly safe. |
...
| Auth | Existing `AuthShell`, `AuthForm`, `BuyerLogin`, `AdminLogin` geometry only | `AuthForm` hanya punya field errors/loading; `BuyerLogin` punya loading/sent; `AdminLogin` masih static/mock. Gunakan field/not-found/root state hanya bila semantik aman; generic/rate/unavailable gaps mengikuti `UXE-011`, bukan panel baru | Redirect/login sesuai surface; invalid buyer token dapat safe `NotFound` | Challenge state hanya dikomposisikan setelah characterization; lihat `UXE-001` dan `UXE-011`. |
...
| `/careers` | Static listing (no recruiting system) | No fabricated network state | **Launch:** each role row is `STATIC` `mailto:careers@fersaku.id?subject=Application:…` (same card geometry; was no-op button) | PUB-230 |
| `/help` | Static local index (no search backend) | Local search/category navigation `STATIC`; no fake network results | **Launch:** client filter on static category cards + query input; support CTA → `/contact` | PUB-230 |
...
| `/status` | **Launch:** non-live informational page + sanitized `GET /v1/status` process identity only (not multi-service operational aggregate) | Never hardcode “operational”, uptime %, incident history, or “last checked”; unavailable → neutral/unavailable, never green default | Service rows `IMPLEMENT` (truthful not_reported / reachable / unavailable); incident accordion empty `STATIC`; full live aggregate deferred until status provider | PUB-220 |
...
| `/docs/api` | Static docs shell aligned with gateway contract narrative | Anchors target real IDs on page | **Launch:** TOC/on-page anchors + CodeBlock copy `STATIC functional`; no `href="#"` | PUB-230/INT-000 |
| API playground component wherever mounted | Isolated sandbox backend/test gateway, never production credential | In explicit `prototype/mock` mode it may retain deterministic labeled no-network response; in `api/live` mode it must call sandbox or be disabled. Never mix modes or show fake IDs as live. | **Launch:** Send `DISABLED` when `publicCatalog` ≠ mock (`title` PUB-230 deferred); mock keeps labeled timer + “Frontend mock • no network request”; tabs/copy `STATIC`; sandbox IMPLEMENT deferred | PUB-230, INT-025 |
...
| Route(s) | Authority | Initial/empty/error contract | Control disposition | Task/gate |
| --- | --- | --- | --- | --- |
| `/register`, `/login` | Seller auth/session/merchant bootstrap | `AuthForm` has field errors/loading only: field `400` and generic invalid may reuse an existing field region; unverified/rate/unavailable/auth require `UXE-011` decision before canary | Submit/reset/logout/session `IMPLEMENT` only with safe negative-state disposition; seller Google control `DISABLED`/`OUT-OF-SCOPE` (AUT-130 launch deferral; mode-gated when auth≠mock); no buyer Google control; fake success forbidden | AUT-100, AUT-120, AUT-130/UI-080/UXE-011 |
...
| `/admin/login` | Admin surface login/session | Snapshot is a static link with default credentials/“Mock access”, not a real auth form. API/live login is blocked until exact same geometry supports real submit, login, and failure semantics or separate `UI-080` approval | Mock access unreachable in API; login/logout `DECISION -> IMPLEMENT/DISABLED` | ADM-100, INT-140/UI-080/UXE-011 |

Default route ownership is fixed: seller reset/verify/invite uses `/login`; admin invitation/login uses `/admin/login`; buyer magic uses `/account/verify`. New auth routes require `UI-080`.
...
| `/account/security` | Buyer sessions + shared auth/security state | Revoke current/all triggers authoritative session outcome; no stale rows | Revoke/logout/password controls `IMPLEMENT` | BUY-130, AUT-120 |
...
| `/dashboard/inventory/[productId]` | Store product/inventory detail and stock items | Reveal recent-auth/no-store; scan/reservation state truthful; only `StockItemsTab` renders existing `TablePagination` | schema/import/reveal/revoke plus detail stock `NumberedPageList`; no copy control exists in snapshot | SEL-240/INT-020 |
...
| `/dashboard/withdrawals`, `/dashboard/withdrawals/new` | Quote/recent-auth/provider state | Unknown outcome reconciled; no optimistic balance/history | quote/reauth/create/filter/detail -> SEL-410 |
...
| `/admin/profile` | Admin profile/session/notifications | Personal media `DISABLED`/`OUT-OF-SCOPE` for launch (INT-175 deferred; store objects forbidden for personal photo); session revocation authoritative | profile/security/notification -> ADM-230; photo stays disabled |
...
| `/admin/withdrawals`, `/admin/withdrawals/[withdrawalId]` | Ledger/provider withdrawal state | reason/idempotency/permission; unknown outcome | review/hold/reject/approve -> ADM-310 |
| `/admin/inventory` | Redacted inventory operations | Reveal only server recent-auth/no-store; foreign/unauthorized safe | existing reveal control -> ADM-320; disabled invalidate/delete remain disabled unless separately characterized; no retry/force/revoke controls added |
...
| `/admin/kyc` | Encrypted KYC metadata + server-decrypted authorized stream | No direct ciphertext object URL; audit/no-store | view/request-info/approve/reject/document -> ADM-340 |
...
| `/admin/providers`, `/admin/system` | Real adapter/queue/health/fee/emergency state | No fake operational status; commands versioned/auth/reason/audit | refresh/preview/emergency -> ADM-370 |
...
| Admin login link/default credentials/“Mock access” | Inputs are uncontrolled defaults and CTA is a plain link; no pending/error region | Mock authority unreachable in API/live; real login remains disabled/blocked until `UXE-011/UI-080` resolves same-geometry form and negative states | ADM-100/INT-140/UI-080 |
...
| Careers role buttons | No handler/link | **Done:** `mailto:careers@fersaku.id` apply links (STATIC) | PUB-230 |
| Help search/category cards | No handler | **Done:** local client filter on static categories | PUB-230 |
| Docs TOC `href="#"` and copy button | No real target/copy | **Done:** real anchors + clipboard copy | PUB-230/INT-000 |
| API playground Send | Timer-generated success and mock IDs | **Launch:** mock-only labeled timer; api/disabled → Send `DISABLED`+title; never fake live; sandbox re-open later | PUB-230 |
| Public status uptime/incidents | Hardcoded operational claims | **Launch:** informational page + sanitized `/v1/status`; no invented uptime/incidents; no green default on failure | PUB-220 |
...
| Seller create/edit/publish/import/reveal/withdraw | Mock/local actions | Exact typed commands, tenant/idempotency/state/audit | SEL-220/240/280/300/410 |
...
| Admin privileged dialogs | Some local persistence/fake operation | Exact typed backend command; reason/idempotency/audit | Matching ADM task |
...
| --- | --- | --- | --- |
| `UXE-001` | Seller/admin/buyer challenge routes for verify/reset/invite/auth | Compose exact existing auth shell/form/dialog on `/login`, `/admin/login`, `/account/verify`; no new route/design | Pre-authorized constraint by 00; AUT-120/ADM-100 |
...
| `UXE-007` | Static/no-op marketing controls | Implement genuine static behavior in same element or disabled composition; removal/new copy/design requires UI-080 | **Closed for PUB-230 surfaces** (help/careers/docs/playground); other marketing no-ops still inventory-driven |
...
- backend direct-request tests prove ownership/tenant/permission/CSRF/idempotency independently of UI;
