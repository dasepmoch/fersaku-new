...
| `/contact` | Backend contact command or disabled launch decision | Current `ContactPage` has no field-error/pending/general-error region; use `UXE-010` (existing form composition only) or keep submit disabled until UI-080, and `sent` only after accepted backend response | Submit `DECISION -> IMPLEMENT/DISABLED`; “kirim lagi” resets form only after real success | PUB-200/UI-080 |
...
| `/account/profile` | Buyer profile + preference revision | Keep form on `400/409`; refetch after conflict; avatar lifecycle separate | Save/preferences `IMPLEMENT`; avatar/photo `DISABLED` until INT-175; “Mulai perubahan email” has no handler/modal, so `DISABLED/OUT-OF-SCOPE` until approved dual-confirm composition/UI-080 | BUY-120/AUT-120, INT-175/UI-080 |
...
| `/admin/profile` | Admin profile/session/MFA/notifications | Personal media via INT-175; session revocation authoritative | profile/photo/security/notification -> ADM-230 |
...
| Seller Google button | Seller `AuthShell` visual button without OAuth action | Implement secure OAuth or disabled capability; never no-op. BuyerLogin has no such control and must not gain one in wiring. | AUT-130 |
...
| Contact submit | `setSent(true)` locally | Backend accepted response before success, or disabled | PUB-200 |
...
| Buyer profile save/preferences/photo | Local saved toggles/no-op photo | Revisioned backend save; personal media INT-175 or disabled | BUY-120 |
...
| `UXE-010` | Contact form has no field-error/pending/general-error region | First use existing form elements with an already approved shared state composition; otherwise keep command disabled and request UI-080. Do not add ad-hoc inline copy in wiring PR. | Pending; PUB-200/UI-080 |
