...
| Seller webhook endpoints | webhooks screen | `GET /v1/stores/{storeId}/webhooks/`, `POST /v1/stores/{storeId}/webhooks/`, `PATCH /v1/stores/{storeId}/webhooks/{id}` | M | Router/handler use `{id}`; freeze slash policy; SSRF protection, schema. | SEL-320/INT-000 |
| Delivery history/test | webhooks screen | `GET /v1/stores/{storeId}/webhooks/deliveries`, `POST /v1/stores/{storeId}/webhooks/{id}/test` | U | Router/handler use `{id}`; pagination/status/idempotency. | SEL-320/INT-000 |
| Secret rotate/claim | webhooks screen | `POST /v1/stores/{storeId}/webhooks/{id}/secret-rotation-requests`, `POST /v1/stores/{storeId}/webhooks/{id}/secret-claims/{claimId}/exchange` | U | Router/handler use `{id}`; one-time component memory. | SEL-320/INT-000 |
