...
The note must use the handoff template in `08-AGENT-EXECUTION-RUNBOOK.md`, link the exact commit/CI run/test artifacts, and never contain raw credentials, tokens, security codes (unused), document bytes, signed URLs, provider signatures, or unnecessary PII. Use sanitized hashes/metadata and test assertions instead.
