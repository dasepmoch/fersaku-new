...
export function Withdrawals() {
  ...
  const unlockLabel = lock.unlockedAt
    ? new Date(lock.unlockedAt).toLocaleString("id-ID", {
        dateStyle: "medium",
        timeStyle: "short",
          ...
          <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:p-6">
            ...
            <div>
              ...
              <p className="mt-1.5 max-w-3xl text-[10px] leading-5 text-[#7c662c]">
                Penarikan terkunci hingga <b>{unlockLabel}</b> demi alasan
                keamanan setelah perubahan rekening bank tujuan.
              </p>
            </div>
            <div className="rounded-xl border border-[#dfbf61] bg-white/55 px-4 py-3 text-center sm:ml-auto">
              ...
              <b className="mt-1 block text-sm text-[#58420e]">
                {lock.remainingLabel ?? "Menunggu pembukaan"}
              </b>
            </div>
          </div>
            ...
            <tbody>
              {pageRows.map((r) => (
                <tr key={r.id} className="hairline border-t text-xs">
                  ...
                  <td>
                    <FinanceSourceBadge source={r.source} />
                  </td>
                  <td>{r.bankLabel}</td>
                  <td>
