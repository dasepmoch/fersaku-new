...
session authentication recent step-up proof for sensitive admin/seller operations
X-Audit-Reason human/operator reason, required for privileged actions
```

No bearer auth/bootstrap token, auth proof, credential, QR payload, bank
...
- Seller/API access suspension are independent. Admin impersonation is
 server-issued, reason/auth/TTL/audit bound, read-only by default, and has no
...
- Seller API keys use request + owner/recent-auth one-time claim; admin/support
