...
```text
Register/login
   -> email verification/session/MFA sesuai policy
...
| ------------------------------------------------- | ----------------------------------------------------------------------------- |
| `/login`, `/register`                             | seller identity, verification, password/session/MFA lifecycle                 |
...
| `/dashboard/settings`                             | personal/business profile, bank, password/MFA, notification preferences       |
...
- Argon2id dan maintained TOTP library.
...
- id, user, surface (`BUYER`, `SELLER`, `ADMIN`), token hash, created/last seen/expiry/revoked, MFA proof time, IP/UA hash, device label.
- Raw token never stored; rotate at login, password/MFA change, privilege elevation, impersonation.

`auth_challenges`:

- purpose, token hash, expiry, attempts, consumed time.
- Used for email, dual-confirmation email change, magic link, reset, MFA, and
...
- Reveal is explicit **per item**, permissioned, recent-MFA-gated where policy
...
- Sensitive change requires permission, fresh MFA, reason, idempotency, audit.
...
- Publish requires permission, recent MFA for critical/compliance priority,
...
Password reset revokes sessions. Admin console requires MFA. Login rotates session.
...
active merchant, enabled API switch, recent-MFA/reason authorization, and an
...
AUTH_MFA_REQUIRED, AUTH_CSRF_INVALID, FORBIDDEN,
...
add an authenticated matching seller, recent MFA, issuance-request version,
...
Password/email/MFA changes require recent proof appropriate to the current
principal, rotate sessions, and notify the old verified address. Recovery codes
...
Secret reveal: explicit item ID + permission + recent MFA policy + reason +
...
1. The authenticated canonical merchant owner calls
   ...
   (`INITIAL_ISSUE` or `ROTATE`), `Idempotency-Key`, recent MFA, expected
...
3. The same seller calls the claim exchange with the raw claim token in the POST
   body plus recent MFA. In one transaction the server locks request/capability,
...
4. An expired/consumed claim reveals nothing and cannot be replayed. A lost API
   ...
   reauthenticates/repeats recent MFA against the still-authorized issuance
...
recent-MFA/version bound, consumes the claim atomically, and reveals the
...
fresh MFA by policy, and audit. It reprocesses the durable stored event through
...
permission, recent MFA, reason, expected predecessor version, and idempotency;
...
scope, `fulfillment.force` permission, fresh MFA, reason, and idempotency. It
...
- admin inventory reveal is per exact item, recent-MFA/reason gated and
...
break-glass-safe administrator, or assign an invited user before email/MFA
...
account collision policy, and staff MFA requirement without revealing whether
...
- [ ] Rotate session at login, privilege change, MFA, password reset, and impersonation start.
...
- [ ] Admin requires MFA; sensitive seller/admin action can require recent MFA proof.
- [ ] Recovery codes hashed, one-time, never returned again after initial display.
...
- [ ] Issuance/claim/rotation is authenticated, recent-MFA/version/idempotency
...
- [ ] Role/invitation commands enforce anti-escalation in the use case and DB
      ...
      required MFA. Privilege changes rotate/revoke affected sessions.
...
effective time, idempotency key, reason, fresh MFA, audit event, and rollback.

### 11.5 Impersonation security

- [ ] Implement explicit
      `POST /v1/admin/users/{userId}/impersonation` plus the merchant-owner
      resolver endpoint. Start requires `impersonation.start`, fresh MFA,
      reason/ticket, exact non-admin target user, scope, TTL, and idempotency.
...
- [ ] All other mutation methods/commands are default-denied during
      ...
      merchant/user status or role, auth/email/password/MFA/session, admin
...
| ------------------------ | ---------------------------------------------------------------------------------- |
| Credential stuffing      | rate limit, generic errors, MFA/admin lock, alert                                  |
...
| Impersonation abuse      | MFA/reason/scope/TTL/banner/immutable actor/audit                                  |
...
| Role escalation/invite   | delegable-permission subset, system-role immutability, MFA/session rotation        |
...
- Auth failure/rate-limit/MFA/impersonation start/end.
...
- seller API-key request/claim and endpoint-secret claim are one-time,
  recent-MFA/owner/version bound; admin never receives raw secret and the two
...
- seller credential/endpoint-secret request + fragment/bootstrap claim exchange
  reveals each raw secret once to the matching recent-MFA seller; admin, replay,
...
27. user changes password/email through recent proof and dual confirmation,
    regenerates one-time MFA recovery codes, and revokes other sessions; an
...
28. admin creates a delegable custom role/invitation -> assignee verifies email
    and MFA -> privilege/session changes take effect; self-escalation, immutable
...
- [ ] Admin MFA/RBAC/bootstrap account/break-glass verified.
...
- [ ] Record session/MFA/impersonation policy and retention owner.
...
- [ ] MFA admin enrollment/verify/recovery/recent proof.
...
- [ ] Implement MFA disable and recovery-code regeneration with fresh MFA,
      one-time display, hashed storage, consumption/replay tests, and security
      notification.
...
email, MFA, and revoke-all changes invalidate exactly the documented sessions
...
- [ ] Implement hashed, email-bound, single-use, expiring/revocable staff and
      ...
      deterministically and requires verified email plus admin MFA policy before
...
| Impersonation             | explicit user target; MFA/TTL; exact support allowlist; full absent          |
